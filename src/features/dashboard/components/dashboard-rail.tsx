...
export function DashboardRail({
  ...
  return (
    <>
      <aside
        className="fixed bottom-3 left-3 top-3 z-[60] hidden flex-col gap-1 overflow-hidden rounded-[18px] bg-brand-deep px-3.5 py-[18px] shadow-[6px_0_30px_rgba(20,21,12,0.18)] transition-[width] duration-200 ease-out md:flex"
        onMouseEnter={openRail}
        onMouseLeave={closeRail}
        style={{ width: railOpen ? 248 : 76 }}
      >
        <Link
          className="mb-3 flex h-11 flex-none items-center gap-3 rounded-[11px] pl-2.5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-spring"
          href="/management"
        >
        ...
        <div className="flex h-12 flex-none items-center gap-3 px-2">
          <div className="flex h-8 w-8 flex-none items-center justify-center rounded-[9px] bg-brand-deep text-[13px] font-bold text-brand-spring">
        ...
        <button
          aria-label="로그아웃"
          className="flex h-11 flex-none items-center gap-3 rounded-[11px] px-3 text-on-brand-deep/80 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-spring"
      ...
      <button
        ...
        className="fixed right-4 top-4 z-[80] inline-flex h-11 w-11 items-center justify-center rounded-[14px] bg-brand-deep text-brand-spring shadow-[0_8px_24px_rgba(20,21,12,0.2)] transition hover:bg-brand-deep focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-spring md:hidden"
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-[70] md:hidden">
          ...
          <aside
            ...
            className="absolute bottom-3 right-3 top-3 grid w-[min(320px,calc(100vw-1.5rem))] grid-rows-[auto_auto_1fr_auto] overflow-hidden rounded-[22px] bg-brand-deep p-4 shadow-[0_18px_60px_rgba(20,21,12,0.3)]"
            id="mobile-dashboard-menu"
          >
            <Link
              className="flex h-12 items-center gap-3 rounded-[13px] pr-12 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-spring"
              href="/management"
              onClick={() => setMobileMenuOpen(false)}
            >
              <span className="flex h-9 w-9 flex-none items-center justify-center rounded-[11px] bg-brand-spring font-display text-lg font-bold text-brand-deep">
                L
              </span>
            ...
            <div className="mt-4 rounded-[14px] bg-black/20 px-3 py-3">
            ...
            <button
              aria-label="로그아웃"
              className="mt-4 flex h-11 items-center gap-3 rounded-[13px] px-3 text-sm font-semibold text-on-brand-deep/80 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-spring"
              disabled={logoutDisabled}
              onClick={onLogout}
              type="button"
            >
...
function MobileDrawerLink({
  ...
  return (
    <Link
      ...
      className={cn(
        "flex h-11 items-center gap-3 rounded-[13px] px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-spring",
        active
...
function RailLink({
  ...
  return (
    <Link
      aria-current={active ? "page" : undefined}
      className={cn(
        "flex h-[46px] flex-none items-center gap-3.5 rounded-[11px] px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-spring",
        active
          ? "bg-brand-spring text-brand-deep"
          : "text-on-brand-deep/80 hover:bg-white/10 hover:text-white",
      )}
