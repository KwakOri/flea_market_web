...
정산 페이지가 **바깥 패널(`panelVariants`) 안에 또 카드(차트·표·히스토리)를 품는** "컨테이너 안 컨테이너" 이중 프레임이라 다른 페이지와 톤이 달랐음. → **바깥 패널 제거**로 통일(사용자 결정).
- `settlement-screen`: `SettlementPreviewPanel`을 감싸던 바깥 `section(panelVariants)` 제거.
- `settlement-preview-panel`: 루트 `p-4` 제거 + `gap-[18px]`→`gap-6`(앱 표준), 로딩/빈 상태는 캔버스에 뜨지 않게 카드로 감쌈.
- 결과: KPI·확정 폼·차트·미리보기 표·히스토리가 **캔버스 위 개별 카드**로 배치(참가부스 화면과 동일한 '카드 온 캔버스' 패턴).
- **참가부스 드릴다운**(부스 행 클릭 → `/settlements/[participantId]`): `SettlementPreviewPanel` 재사용이라 위 평탄화가 그대로 적용됨(화면 확인 완료 — 이중 프레임 없음).
- **회차 상세**(히스토리 회차 클릭 → `/settlements/versions/[id]`): 별도 standalone 화면이나 이미 '카드 온 캔버스' 구조(이중 프레임 없음). 콘텐츠 간격만 `gap-[18px]`→`gap-6`으로 통일. (mock에 확정 정산이 없어 화면 미확인, 코드/타입체크 완료)
...
- `a2d0671` 정산 회차 상세: 카드 간격 gap-6 통일
- `2f1bb56` 정산 레이아웃: 바깥 패널 제거해 카드 온 캔버스로 통일
...
_마지막 업데이트: 참가부스 패널 간격 + 정산 레이아웃 통일 반영 (커밋 28개, 작업 트리 clean)_
